...
from agents.user_config import UserAgent
# from agents.user_config import summarize_function_call_stats  # <-- Add this import

# from agents.new_gemini_agent import NewGeminiAgent
# UserAgent = NewGeminiAgent
import sys
import os
...
log_path = "run_output_task1.txt"  # 또는 "logs/run_output.txt"
sys.stdout = TeeLogger(log_path)
sys.stderr = TeeLogger(log_path, mode='a')


def summarize_function_call_stats(gold_total, pred_total, correct_match):
    """
    Prints summary statistics for function call evaluation.
    """
    print("\n[SUMMARY] Function Call Evaluation")
    print(f"- Total Gold Calls (expected): {gold_total}")
    print(f"- Total Model Calls (actual): {pred_total}")
    print(f"- Correct Matches: {correct_match}")
    if gold_total > 0:
        accuracy = correct_match / gold_total * 100
...
if __name__ == '__main__':
    ...
    for conv_idx, conversation in tqdm(enumerate(data_set)):
        ...
        for turn_idx, turn in enumerate(conversation.turns):
            ...
            # Function call stats
            gold_total += len(gold_functions)
            pred_total += len(cur_turn_exec.function_call_stats)
            for pred in cur_turn_exec.function_call_stats:
                if any(pred['name'] == gold['name'] and pred['parameters'] == gold['parameters'] for gold in gold_functions):
                    correct_match += 1
    ...
    # Call summary
    summarize_function_call_stats(gold_total, pred_total, correct_match)
